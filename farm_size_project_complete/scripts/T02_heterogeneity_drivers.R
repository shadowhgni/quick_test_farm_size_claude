...
for(var in c('harv_area', 'production', 'value')){
  ...
  cassa_ssa <- each_crop[[paste0('CASS', var_code)]] 
  yams_ssa <- each_crop[[paste0('YAMS', var_code)]]
  pota_ssa <- each_crop[[paste0('POTA', var_code)]]
  swpo_ssa <- each_crop[[paste0('SWPO', var_code)]]
  root_ssa <- sum(c(cassa_ssa, yams_ssa, pota_ssa, swpo_ssa), na.rm = T); names(root_ssa) <- paste0('root', var_code)
  
  grou_ssa <- each_crop[[paste0('GROU', var_code)]]
  beans_ssa <- each_crop[[paste0('BEAN', var_code)]]
  
  legm_ssa <- sum(c(each_crop[[paste0('BEAN', var_code)]], each_crop[[paste0('CHIC', var_code)]], each_crop[[paste0('COWP', var_code)]],
                    each_crop[[paste0('PIGE', var_code)]], each_crop[[paste0('LENT', var_code)]], each_crop[[paste0('OPUL', var_code)]], 
                    each_crop[[paste0('SOYB', var_code)]], each_crop[[paste0('GROU', var_code)]] ), na.rm = T); names(legm_ssa) <- paste0('legumes', var_code)
  
  non_food_ssa <- sum(c(each_crop[[paste0('SUGC', var_code)]], each_crop[[paste0('SUGB', var_code)]], each_crop[[paste0('COTT', var_code)]], each_crop[[paste0('OFIB', var_code)]],
                        each_crop[[paste0('ACOF', var_code)]], each_crop[[paste0('RCOF', var_code)]], each_crop[[paste0('COCO', var_code)]],
                        each_crop[[paste0('TEAS', var_code)]], each_crop[[paste0('TOBA', var_code)]]), na.rm = T); names(non_food_ssa) <- paste0('non_food', var_code)
