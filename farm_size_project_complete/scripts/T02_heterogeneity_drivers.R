...
theor_app2 <- tryCatch({
  ...
  # Drop ID column if present (added by terra::extract), then bind ssa columns
  .tfa <- if ('ID' %in% names(theor_farms_application)) dplyr::select(theor_farms_application, -ID) else theor_farms_application
  .tfa |>
